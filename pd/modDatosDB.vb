...
Module modDatosDB
    ...
    Public Function leerEstilos() As Boolean
        ...
        Dim adaptador As New OleDb.OleDbDataAdapter("Select * from estilos", conexionMdb)
        Dim ds As New DataSet
        tblEstilos = New DataTable
        adaptador.Fill(tblEstilos)
        If tblEstilos.Rows.Count = 0 Then
            Return False
        Else
            Return True
        End If
    ...
    Public Function actuEstilos(ByVal nombreEstilo As String, ByVal tipoDeLetra As String, ByVal altura As String)
        If conexionMdb Is Nothing Then
            If Not conectar() Then
                MsgBox("No se pudo establecer la conexion.")
                Exit Function
        ...
        Try
            ...
            '--------------------------
            sql = "UPDATE estilos SET tipoLetra='" & tipoDeLetra & "', altura=" & CDbl(altura) & " WHERE nombreEstilo='" & nombreEstilo & "'"
            Cmd.CommandText = sql
    ...
    Public Sub guardarMedidaLadoParcela(ByVal elTexto As VectorDraw.Professional.vdFigures.vdText)

        Dim Random As New Random()
        idEntidad = elTexto.Handle.Value + CInt(rnd.Next(999999))
        ...
        'tabla entidades
        '---------------------
        sql = "INSERT into entidades (idEntidad, tipoEntidadLogica, tipoEntidadGeometrica) values(" & idEntidad & ", 'Medida Lado Pl', 'Texto')"
        Cmd.CommandText = sql
        Cmd.Connection = conexionMdb
        Cmd.ExecuteNonQuery()
        ...
        '-----------------------
        '20/05/2013
        sql = "INSERT into atributosLogicos (idEntidad, medida_lado_parcela, idPl) values (" & idEntidad & ", '" & elTexto.TextString & "', " & _parcela.idParcela & ")"
        Cmd.CommandText = sql
        Cmd.Connection = conexionMdb
        Cmd.ExecuteNonQuery()
        ...
        Try
            ...
            i = i + 1
            puntox = elTexto.InsertionPoint.x.ToString.Replace(",", ".")
            puntoy = elTexto.InsertionPoint.y.ToString.Replace(",", ".")
            puntoz = elTexto.InsertionPoint.z.ToString.Replace(",", ".")
            sql = "INSERT into coordenadas (idEntidad,X,Y,Z, angulo) values(" & idEntidad & "," & puntox & "," & puntoy & "," & puntoz & "," & Math.Round(elTexto.Rotation, 2) & ")"
            Cmd.CommandText = sql
            Cmd.Connection = conexionMdb
            Cmd.ExecuteNonQuery()
        Catch ex As Exception
            MsgBox("Error al guardar las coordenadas de la madida de un lado de la parcela.")
        End Try
    ...
    Public Sub borrarEntidad(ByVal idEntidadABorrar As Integer)
        ...
        If ds.Tables("tblEntidad").Rows.Count = 1 Then
            ...
            If ds.Tables("tblEntidad").Rows(0).Item("tipoEntidadLogica") = "Parcela" Then
                '----> medida_lado_parcela:
                Cmd.CommandText = "select medida_lado_parcela, idEntidad from atributosLogicos where not medida_lado_parcela="""" and idPl=" & idEntidad
                Cmd.Connection = conexionMdb
                adaptador.SelectCommand = Cmd
                adaptador.Fill(ds, "tblLadoParcela")
                For Each registro As DataRow In ds.Tables("tblLadoParcela").Rows
                    sql = "DELETE * FROM entidades WHERE idEntidad=" & registro("idEntidad")
                    Cmd.CommandText = sql
                    Cmd.Connection = conexionMdb
                    Cmd.ExecuteNonQuery()
            ...
            If ds.Tables("tblEntidad").Rows(0).Item("tipoEntidadLogica") = "Muro" Then
                '----> 
                Cmd.CommandText = "select idEntidad from atributosLogicos where not medida_muro="""" and idMuro=" & idEntidad
                Cmd.Connection = conexionMdb
                adaptador.SelectCommand = Cmd
                adaptador.Fill(ds, "tblMuro")
                For Each registro As DataRow In ds.Tables("tblMuro").Rows
                    sql = "DELETE * FROM entidades WHERE idEntidad=" & registro("idEntidad")
                    Cmd.CommandText = sql
                    Cmd.Connection = conexionMdb
                    Cmd.ExecuteNonQuery()
                    ...
                    sql = "DELETE * FROM atributosGeometricos WHERE idEntidad=" & registro("idEntidad")
                    Cmd.CommandText = sql
                    Cmd.Connection = conexionMdb
                    Cmd.ExecuteNonQuery()
                    ...
                    For Each entidad As VectorDraw.Professional.vdPrimaries.vdFigure In frmPpal.VdF.BaseControl.ActiveDocument.ActiveLayOut.Entities
                        If entidad.ToolTip <> "" Then
                            Dim auxiliar() As String = Split(entidad.ToolTip, ", id:")
                            If auxiliar.Count > 1 Then
                                If CInt(Trim(auxiliar(1))) = idEntidad Then
                                    entidad.Deleted = True
                                End If
                            End If
                        End If
            ...
            If ds.Tables("tblEntidad").Rows(0).Item("tipoEntidadLogica") = "Ancho muro" Then
                '----> 
                Cmd.CommandText = "select idEntidad from atributosLogicos where not medida_muro="""" and idMuro=" & idEntidad
                Cmd.Connection = conexionMdb
                adaptador.SelectCommand = Cmd
    ...
    Public Sub agregarDatosMacizoBD_anchoMuro(ByVal elTexto As VectorDraw.Professional.vdFigures.vdText)
        ...
        '-------------------
        idEntidad = elTexto.Handle.Value + CInt(rnd.Next(999999))
        _muro.idTxtAnchoMuro = idEntidad
        ...
        If ds.Tables("tblEntidad").Rows.Count = 0 Then
            ...
            'tabla entidades
            '---------------------
            sql = "INSERT into entidades (idEntidad, tipoEntidadLogica, tipoEntidadGeometrica) values(" & idEntidad & ", 'Ancho muro', 'Texto')"
            Cmd.CommandText = sql
            Cmd.Connection = conexionMdb
            Cmd.ExecuteNonQuery()
            ...
            'atributosLogicos
            '----------------------
            sql = "INSERT into atributosLogicos (idEntidad, medida_muro, idMuro) values (" & idEntidad & ", '" & elTexto.TextString & "', " & _muro.idMuro & ")"
            Cmd.CommandText = sql
            Cmd.Connection = conexionMdb
            Cmd.ExecuteNonQuery()
            ...
            'tabla coordenadas
            '-------------------------------------------------------------
            sql = "INSERT into coordenadas (idEntidad, x, y, z, angulo) values (" & idEntidad & ", " & Math.Round(elTexto.InsertionPoint.x, 4) & ", " & Math.Round(elTexto.InsertionPoint.y, 4) & ", " & Math.Round(elTexto.InsertionPoint.z, 2) & ", " & Math.Round(elTexto.Rotation, 2) & ")"
            Cmd.CommandText = sql
            Cmd.Connection = conexionMdb
            Cmd.ExecuteNonQuery()
    ...
    Public Sub agregarDatosMacizoBD_muros(ByVal datos As String, ByVal laPolilinea As VectorDraw.Professional.vdFigures.vdPolyline, ByVal tipoEntidadLogica As String)
        ...
        If ds.Tables("tblEntidad").Rows.Count = 0 Then
            ...
            'atributosLogicos
            '----------------------
            'sql = "INSERT into atributosLogicos (idEntidad, medida_muro, idMuro) values (" & idEntidad & ", '" & datos & "', " & idEntidad & ")"
            'Cmd.CommandText = sql
            'Cmd.Connection = conexionMdb
            'Cmd.ExecuteNonQuery()
